...
$where = "WHERE subjects.school_id='$school_id'";
$params = [];

// Join subject_teacher and users (teacher)
$join = "LEFT JOIN subject_teacher ON subjects.id = subject_teacher.subject_id 
         LEFT JOIN users ON subject_teacher.user_id = users.id AND users.role='teacher'";

// Search by subject fields or teacher name
if ($search !== '') {
  $where .= " AND ( subjects.name LIKE :search OR subjects.type LIKE :search OR subjects.code LIKE :search OR users.name LIKE :search )";
  $params[':search'] = "%$search%";
}

// Always filter by school_id in all queries
$sql = "SELECT subjects.*, 
        GROUP_CONCAT(DISTINCT users.name SEPARATOR ', ') AS teacher_names
        FROM subjects $join $where 
...
$operation = "Generated a subject Report on $current_date";
...
<head>
  ...
  <title><?php echo $app_name; ?> | Subject Report</title>
          ...
          <div class="col-12">
            <div class="card" style="box-shadow:0 2px 12px rgba(0,0,0,0.06);">
              <div class="card-header bg-primary text-white">
                <h5 class="mb-0">Subject Report</h5>
              </div>
                <div class="card-body">
                <!-- Custom Search Box -->
                <form class="custom-search-box" method="get" action="">
                  <input type="text" name="search" placeholder="Search subjects or teachers..." value="<?php echo htmlspecialchars($search); ?>" />
                  <button type="submit"><i class="fa fa-search"></i> Search</button>
                  <?php if ($search !== ''): ?>
                  <a href="subject_report.php" class="btn btn-secondary" style="border-radius:20px;padding:6px 18px;font-size:15px;">Reset</a>
                  <?php endif; ?>
                </form>
                <!-- Printable Area -->
                <div id="printable-area">
                  <div class="report-title"><?php echo $app_name; ?> - Subject Report</div>
                  ...
                  foreach ($data as $row): ?>
                    ...
                    <td><?php echo htmlspecialchars($row['teacher_names'] ?: 'N/A'); ?></td>
                    </tr>
                  <?php $cnt++; endforeach; else: ?>
                    <tr>
                    <td colspan="5" class="text-center text-danger">No subject found.</td>
                    </tr>
                  <?php endif; ?>
                  </tbody>
                  </table>
...
<html>
<head>
  <title>Print - Subject Report</title>
  <style>
